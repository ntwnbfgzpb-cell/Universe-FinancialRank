...
function SettingsDialog({ close, reload }) {
  ...
  const [backups,setBackups]=useState([]),[backupMessage,setBackupMessage]=useState(""),[backupBusy,setBackupBusy]=useState(false);
  const loadBackups=()=>endpoints.backups().then(setBackups).catch((error)=>setBackupMessage(error.message));
  useEffect(()=>{loadBackups()},[]);
  const createBackup=async()=>{setBackupBusy(true);setBackupMessage("");try{const result=await endpoints.createBackup();setBackupMessage(`${result.message}｜${result.backup_id}`);await loadBackups()}catch(error){setBackupMessage(error.message)}finally{setBackupBusy(false)}};
  const restoreBackup=async(backup)=>{if(!window.confirm(`確定還原 ${backup.backup_id}？\n目前資料庫會先自動建立安全備份，還原後請重新整理程式。`))return;setBackupBusy(true);setBackupMessage("");try{const result=await endpoints.restoreBackup(backup.backup_id);setBackupMessage(`${result.message}｜安全備份：${result.safety_backup_id}`);await loadBackups();reload()}catch(error){setBackupMessage(error.message)}finally{setBackupBusy(false)}};
  ...
  return <div className="modalBackdrop" onMouseDown={close}><section className="settingsDialog" onMouseDown={(event)=>event.stopPropagation()}><div className="dialogTitle"><div><h2>桌面程式設定</h2><p>設定與備份只保存在這台電腦。</p></div><button onClick={close}>×</button></div><label className="settingRow"><div><b>自動重新整理</b><span>每 30 秒檢查本機 API 與最新快照</span></div><input type="checkbox" checked={autoRefresh} onChange={(e)=>setAutoRefresh(e.target.checked)}/></label><label className="settingRow"><div><b>宇宙視覺動態</b><span>啟用低速 Liquid Orb 光影；關閉後完全靜止</span></div><input type="checkbox" checked={motion} onChange={(e)=>setMotion(e.target.checked)}/></label><div className="backupManager"><div className="backupTitle"><div><b>資料庫備份與還原</b><span>每份備份均檢查 SHA-256 與 SQLite integrity_check；還原前會再建立安全備份。</span></div><button className="primary" disabled={backupBusy} onClick={createBackup}>{backupBusy?"處理中…":"立即建立備份"}</button></div>{backupMessage&&<p className="backupMessage">{backupMessage}</p>}<div className="backupList">{backups.length?backups.map((backup)=><article><div><b>{backup.backup_id}</b><span>{backup.created_at||"無法讀取建立時間"} · {backup.snapshots??0} 個快照 · {(Number(backup.size||0)/1024/1024).toFixed(1)} MB</span><code>{backup.verified?`${backup.sha256?.slice(0,20)}…`:backup.error}</code></div><button disabled={backupBusy||!backup.verified} onClick={()=>restoreBackup(backup)}>還原</button></article>):<p>尚未建立本機備份。</p>}</div></div><div className="engineInfo"><code>http://127.0.0.1:8765</code><span>僅限本機 loopback，不對外開放。</span></div><div className="dialogActions"><button onClick={close}>取消</button><button className="primary" onClick={save}>儲存設定</button></div></section></div>;
}
